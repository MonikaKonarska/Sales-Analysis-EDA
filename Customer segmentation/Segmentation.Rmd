...
clusterPc3                <- prcomp(datatoAlgorythmScaled, center = FALSE, scale. = FALSE)$x %>% as.data.frame()
...
head(gather(data.frame(datatoAlgorythmScaled[clusters3$cluster == 1,])))

tmp <- aggregate(datatoAlgorythmScaled, by = list(clusters3$cluster), mean)



daneBeh <- basketCustomerInformation
daneBeh$cluster <- factor(clusters3$cluster)

daneSegent <- daneBeh %>%
   select(c("cluster" ,"minBasket","maxBasket","totalBasket", "nBasket", "Gender", "Age", "Stay_In_Current_City_Years", "Marital_Status")) %>%
   group_by(cluster,Gender, Age, Stay_In_Current_City_Years, Marital_Status) %>%
   summarise(avgminBasket = mean(minBasket),
             avgmaxBasket = mean(maxBasket),
             avgtotalBasket = mean(totalBasket),
             avgnBasket = mean(nBasket))

ggplot(daneSegent, aes(x=Marital_Status, y=avgminBasket, fill=cluster)) +
  geom_bar(stat = 'identity')

daneBeh$totalCat1 <- daneBeh[,13]

daneBeh %>%
   select(cluster ,minBasket,maxBasket,totalBasket, nBasket, totalCat1) %>%
   group_by(cluster) %>%
   summarise(avgminBasket = mean(minBasket),
             avgmaxBasket = mean(maxBasket),
             avgtotalBasket = mean(totalBasket),
             avgnBasket = mean(nBasket))+
  ggplot(aes(x=Gender, y=avgminBasket, fill=cluster)) +
  geom_bar(stat = 'identity')
...
ggplot(daneBeh, aes(x=cluster, y=avgBasket, fill=cluster)) +  geom_violin()
ggplot(daneBeh, aes(x=cluster, y=maxBasket, fill=cluster)) +  geom_violin()


ggplot(daneBeh, aes(x=Age, fill = cluster)) +
  geom_bar() +
...
ggplot(daneBeh, aes(Age))+
  geom_bar(daneBeh, aes(fill = cluster))
...
plot_str(daneBeh)
plot_str(daneBeh, type = "r")
introduce(daneBeh)
plot_intro(daneBeh)
plot_intro(basketCustomerInformation)
plot_missing(basketCustomerInformation)
plot_bar(basketCustomerInformation)
plot_histogram(basketCustomerInformation)

plot_correlation(na.omit(basketCustomerInformation), maxcat = 5L)
