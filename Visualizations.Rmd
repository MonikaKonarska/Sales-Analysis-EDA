...
cities <- levels(dataCustomer$City_Category)
dataCustomer$order <- as.numeric(dataCustomer$City_Category)/100

ggplot( data = dataCustomer, aes(x = factor(order), fill = Gender))+
  ...
  ggtitle("Customers by gender in cities") +
  theme(plot.title = element_text(hjust = 0.5))
...
dataCustomer %>% select(c("Age", "Gender")) %>%
  group_by(Age, Gender) %>%
  count() %>%
  ggplot( aes(x = Age, y = n, fill = Gender))+
  geom_bar( data =. %>% filter(Gender == "F"), stat = "identity") +
  geom_bar( data =. %>% filter(Gender == "M"), stat = "identity") +
  scale_y_continuous(breaks = seq(-1500, 1500, 500))+
  coord_flip()


                     labels = paste0(as.character(c(seq(1.5, 0, -0.5), seq(0.5, 1.5, 0.5)))))+
  coord_flip()
    
    
    labels = paste0(as.character(c(seq(1500, 0, -500), seq(500, 1500, 500))), "m")) + 
  coord_flip()
