...
dataCustomer <- dataCustomer %>%
  mutate(purchaseQuantile = cut(sumOfPurchase, breaks = quantile(dataCustomer$sumOfPurchase, probs = seq(0, 1, 0.25))),
         purchaseQuantile = case_when(dataCustomer$purchaseQuantile == "(566,6.92e+03]" ~ "566$-6920$",
...
levels(dataCustomer$purchaseQuantile) <- 


dataCustomer$purchaseQuantile <- factor(dataCustomer$purchaseQuantile, levels = c("566$-6920$","6920$-8600$","8600$-12100$","12100$-23900$"))





tab1<-with(daneKlienci,table(Age,PurchaseQuantile))
chisq.test(tab1)
plot(ca(tab1), arrows = c(TRUE, FALSE))
summary(ca(tab1))


tab2<-with(daneKlienci,table(City_Category,PurchaseQuantile ))
chisq.test(tab2)
plot(ca(tab2), arrows = c(TRUE, FALSE))
summary(ca(tab2))

# zbyt duzo kategorii 
tab3<-with(daneKlienci,table(City_Category,Occupation ))
chisq.test(tab3)
plot(ca(tab3), arrows = c(TRUE, FALSE))
summary(ca(tab3))


tabelaKontyngencji<-with(daneMod,table(Age, Stay_In_Current_City_Years))
chisq.test(tabelaKontyngencji)
plot(ca(tabelaKontyngencji), arrows = c(TRUE, FALSE))
summary(ca(tabelaKontyngencji))
